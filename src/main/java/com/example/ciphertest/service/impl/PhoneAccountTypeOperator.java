...
public class PhoneAccountTypeOperator implements AccountTypeOperator {
    ...
    public BiConsumer<CustomerParam, String> getAccountSetter() {
        return CustomerParam::setPhone;
    }

    @Override
    public Function<CustomerParam, String> getAccountGetter() {
        return CustomerParam::getPhone;
    }

    @Override
    public void checkAccountNotExists(CustomerParam customer) {
        log.info("PhoneAccountTypeOperator checkAccountNotExists : {}", customer.getPhone());
        // 校验手机号码是否已被使用. 若被使用, 则抛异常
    }

    @Override
    public void checkAccountExists(CustomerParam customer) {
        log.info("PhoneAccountTypeOperator checkAccountExists : {}", customer.getPhone());
